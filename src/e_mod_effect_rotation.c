...
static void
_rotation_effect_trans_begin_op(Elm_Transit_Effect *context,
                                Elm_Transit *transit,
                                double progress)
{
   Rotation_Effect_Begin_Context *ctx_begin = (Rotation_Effect_Begin_Context *)context;
   Evas_Coord x, y, w, h;
   double curr;
...
static void
_rotation_effect_trans_begin_context_free(Elm_Transit_Effect *context,
                                          Elm_Transit *transit)
{
   Rotation_Effect_Begin_Context *ctx_begin = (Rotation_Effect_Begin_Context *)context;
   Rotation_Effect_Object *eobj;
   ...
   if (ctx_begin == _rotation_zone->effect->ctx_begin)
     _rotation_zone->effect->ctx_begin = NULL;
...
static void
_rotation_effect_trans_begin_cb_del(void *data,
                                    Elm_Transit *transit)
{
   Rotation_Effect *effect = (Rotation_Effect *)data;

   if (!effect) return;
   if (effect->trans_begin == transit)
     {
        EFFINF("Rotation Begin Trans End", NULL, NULL);
        effect->trans_begin = NULL;
     }
}
...
static void
_rotation_effect_trans_end_op(Elm_Transit_Effect *context,
                              Elm_Transit *transit,
                              double progress)
{
   Rotation_Effect_End_Context *ctx_end = (Rotation_Effect_End_Context *)context;
   Evas_Coord x, y, w, h;
   double curr;
...
static void
_rotation_effect_trans_end_context_free(Elm_Transit_Effect *context,
                                        Elm_Transit *transit)
{
   Rotation_Effect_End_Context *ctx_end;
   Rotation_Effect_Object *eobj;
   ...
   if (ctx_end == _rotation_zone->effect->ctx_end)
     _rotation_zone->effect->ctx_end = NULL;
...
static void
_rotation_effect_trans_end_cb_del(void *data,
                                  Elm_Transit *transit)
{
   Rotation_Effect *effect = (Rotation_Effect *)data;

   if (!effect) return;
   if (effect->trans_end == transit)
     {
        EFFINF("Rotation End Trans Del", NULL, NULL);
        effect->trans_end = NULL;
        //        _rotation_zone->zone->rot.block_count --;
...
_rotation_effect_clear(Rotation_Effect *effect)
{
   ...
   if (effect->trans_begin)
     {
        if (!effect->running)
          elm_transit_del_cb_set(effect->trans_begin, NULL, NULL);
        elm_transit_del(effect->trans_begin);
   ...
   if (effect->trans_end)
     {
        if (!effect->running)
          elm_transit_del_cb_set(effect->trans_end, NULL, NULL);
        elm_transit_del(effect->trans_end);
     }

   if (effect->ctx_begin)
     _rotation_effect_trans_begin_context_free((Elm_Transit_Effect*)effect->ctx_begin, NULL);

   if (effect->ctx_end)
     _rotation_effect_trans_end_context_free((Elm_Transit_Effect*)effect->ctx_end, NULL);
...
{
   if ((!effect->trans_begin) || (!effect->trans_end)) return;
   ...
   elm_transit_go(effect->trans_begin);
   elm_transit_go(effect->trans_end);
}

static void
_rotation_effect_begin_prepare(Rotation_Effect *effect)
{
   if (!effect) return;
   ...
   effect->trans_begin = elm_transit_add();
   elm_transit_del_cb_set(effect->trans_begin, _rotation_effect_trans_begin_cb_del, effect);
   ...
   elm_transit_object_add(effect->trans_begin, effect->ctx_begin->layout);
   elm_transit_smooth_set(effect->trans_begin, EINA_FALSE);
   elm_transit_duration_set(effect->trans_begin, 0.3f);
   elm_transit_effect_add(effect->trans_begin, _rotation_effect_trans_begin_op,
                          effect->ctx_begin, _rotation_effect_trans_begin_context_free);
   elm_transit_tween_mode_set(effect->trans_begin, ELM_TRANSIT_TWEEN_MODE_DECELERATE);
   elm_transit_objects_final_state_keep_set(effect->trans_begin, EINA_FALSE);
...
static void
_rotation_effect_end_prepare(Rotation_Effect *effect)
{
   if (!effect) return;
   if (!effect->targets) return;
   if (!effect->trans_begin) return;
   ...
   effect->trans_end = elm_transit_add();
   elm_transit_del_cb_set(effect->trans_end, _rotation_effect_trans_end_cb_del, effect);
   ...
   elm_transit_object_add(effect->trans_end, effect->ctx_end->layout);
   elm_transit_smooth_set(effect->trans_end, EINA_FALSE);
   elm_transit_duration_set(effect->trans_end, 0.3f);
   elm_transit_effect_add(effect->trans_end, _rotation_effect_trans_end_op,
                          effect->ctx_end, _rotation_effect_trans_end_context_free);
   elm_transit_tween_mode_set(effect->trans_end, ELM_TRANSIT_TWEEN_MODE_DECELERATE);
   elm_transit_objects_final_state_keep_set(effect->trans_end, EINA_FALSE);
}
...
_rotation_effect_cb_zone_rotation_begin(void *data, int type, void *event)
{
   ...
   _rotation_effect_begin_prepare(effect);
...
//        _rotation_zone->zone->rot.block_count ++;
   ...
   if (!effect->trans_end) _rotation_effect_end_prepare(effect);
   if (effect->trans_end)
     {
//        _rotation_zone->zone->rot.block_count ++;
        _rotation_effect_start(effect);
     }
...
_rotation_effect_cb_buffer_change(void *data, int ev_type, void *event)
{
   ...
   effect = (Rotation_Effect *)data;
   if (!effect) return ECORE_CALLBACK_PASS_ON;
   if (!effect->trans_begin) return ECORE_CALLBACK_PASS_ON;
   ...
   if (!effect->trans_end) _rotation_effect_end_prepare(effect);
   if (effect->trans_end) _rotation_effect_start(effect);
